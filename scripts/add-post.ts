...
async function promptVarietyProperties(): Promise<Record<string, unknown>> {
  ...
  await numericOptional(
    "dimension",
    "Dimension (optional, leave blank to skip):",
  );
  await stringOptional(
    "ambient_space",
    'Ambient space, e.g. "P^3" (optional):',
  );
  await numericOptional("degree", "Degree (optional):");
  await stringOptional(
    "singularities",
    "Singularities description (optional):",
  );
  ...
  if (isRationalStr !== "unknown")
    props["is_rational"] = isRationalStr === "yes";

  const kodairaRaw = await input({
    message:
      "Kodaira dimension (optional, use -inf, 0, 1, 2, ... or leave blank):",
  });
  if (kodairaRaw.trim() !== "") {
