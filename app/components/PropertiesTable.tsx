...
function formatValue(
  ...
  value: NonNullable<Properties[keyof Properties]>,
): React.ReactNode {
...
export function PropertiesTable({ properties }: PropertiesTableProps) {
  const rows = PROPERTY_LABELS.filter(
      ...
      <table className="w-full text-sm">
        <tbody>
          {rows.map(({ key, label }, i) => (
            ...
            >
              <td
                className="px-4 py-2.5 text-gray-400 font-medium w-2/5"
              ...
              <td className="px-4 py-2.5 text-gray-800">
                {formatValue(
                  key,
                  properties[key] as NonNullable<Properties[keyof Properties]>,
                )}
              </td>
            </tr>
          ))}
