...
>(items: T[], p: SearchParams): T[] {
  ...
  if (p.dim !== undefined)
    results = results.filter(
      (e) =>
        e.type === "variety" &&
        (e as unknown as VarietyExample).properties?.dimension === p.dim,
    );
  if (p.rational !== undefined)
    results = results.filter(
      (e) =>
        e.type === "variety" &&
        (e as unknown as VarietyExample).properties?.is_rational === p.rational,
    );
  if (p.smooth)
    results = results.filter(
      (e) =>
        e.type === "variety" &&
        (
          e as unknown as VarietyExample
        ).properties?.singularities?.toLowerCase() === "smooth",
    );
  return results;
}
