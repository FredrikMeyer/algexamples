...
test.describe("SPA navigation (client-side data fetching)", () => {
  test("navigates from homepage to example detail without full page reload", async ({
    page,
  }) => {
    ...
    const serverRpcRequests = requests.filter((url) =>
      url.includes("/_server/"),
    );
    expect(serverRpcRequests).toHaveLength(0);
  });

  test("navigates from example detail back to examples list", async ({
    page,
  }) => {
    await page.goto("/");
    await page.waitForLoadState("networkidle");
